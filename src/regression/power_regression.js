class PowerRegression {
    ...
    */
    constructor(optimizer=null) {
        this.iteration = 0;
        this.coefficients = Array(2).fill(0.1);
        this.optimizers = [];
        if(optimizer != null) {
    ...
    */
    predict(x) {
        ...
        return this.coefficients[0] * Math.pow(x, this.coefficients[1]);
    }
    /*
    ...
    */
    gradient(data=[]) {
        ...
        for(var i=0; i<n; i++) {
            gradients[0] += (1.0 / n) * Math.max(1 + (1 / data[i][0]), 1) * (data[i][1] - this.predict(data[i][0]));
    ...
    */
    bestFit(data) {
        SMathJsUtils.isValidNdTupleArray(data, 2);
        var n = data.length, lnxMean = 0.0, lnyMean = 0.0, Sxx = 0.0, Sxy = 0.0;
        for(var i=0; i<n; i++) {
        ...
        for(var i=0; i<n; i++) {
            ...
            Sxy += (Math.log(data[i][0]) - lnxMean) * (Math.log(data[i][1]) - lnyMean);
        }
        this.coefficients[1] = Sxy / Sxx;
        this.coefficients[0] = Math.exp(lnyMean - this.coefficients[1] * lnxMean);
    }
    /*
    ...
    */
    correlationCoefficient(data) {
        SMathJsUtils.isValidNdTupleArray(data, 2);
        var n = data.length, lnxMean = 0.0, lnyMean = 0.0, Sxx = 0.0, Syy = 0.0, Sxy = 0.0;
        for(var i=0; i<n; i++) {
            lnxMean += (1.0 / n) * Math.log(data[i][0]);
            lnyMean += (1.0 / n) * Math.log(data[i][1]);
        }
